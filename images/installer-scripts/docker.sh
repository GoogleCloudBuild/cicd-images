...
# Install docker-in-docker daemon
if [[ $# -lt 1 ]]; then
  INSTAL_DIND=""
else
  INSTAL_DIND=${1#-}
fi
if [[ "$INSTAL_DIND" == "dind" ]]; then
  CONTAINERD_VERSION=$(yq '.build-tools.containerd.version' <$PACKAGES)
  sources=$(yq '.build-tools.docker.sources.[]' <$PACKAGES)
  copy_sources /usr/local/bin $sources
