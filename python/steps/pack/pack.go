...
func installDependenciesForPackage(runner cmd.CommandRunner, logger *zap.Logger) error {
	logger.Info("Installing dependencies for package command")

	commands := []string{
		"install", "build", "wheel", "twine",
	}
	if err := runner.Run(logger, cmd.VirtualEnvPip, commands...); err != nil {
		return err
	}
...
func packagePythonArtifact(runner cmd.CommandRunner, args Arguments, logger *zap.Logger) error {
	...
	if err := runner.Run(logger, cmd.VirtualEnvPython3, args.Command...); err != nil {
...
func pushPythonArtifact(runner cmd.CommandRunner, args Arguments, client auth.HTTPClient, logger *zap.Logger) error {
